...
import { ContractsFooterStrip } from "../../../components/onchain";
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen max-w-full overflow-x-hidden bg-[var(--paper)] text-[var(--ink)] md:grid md:grid-cols-[240px_minmax(0,1fr)]">
    <aside className="hidden h-screen flex-col border-r border-[var(--border)] bg-[var(--surface)] px-4 py-5 md:flex">
      <Link href="/" aria-label="Auralis Finance home" className="block rounded-lg px-2 py-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--teal)]"><Logo className="h-7" priority /><div className="mt-2 flex items-center gap-1.5 text-[11px] text-[var(--text-secondary)]"><span className="h-1.5 w-1.5 rounded-full bg-[var(--emerald)]"/>Mantle Mainnet</div></Link>
      ...
      <div className="rounded-[10px] border border-[var(--border)] bg-[var(--surface-muted)] p-3"><Link href="/docs" className="flex items-center gap-2 text-sm font-medium hover:text-[var(--teal)]"><MessageSquare size={14}/>Need help?</Link><p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">Browse the <Link href="/docs" className="text-[var(--teal)] hover:underline">docs</Link> or ask the <Link href="/app/copilot" className="text-[var(--teal)] hover:underline">Copilot</Link>.</p></div>
    </aside>
    <main className="min-w-0"><header className="sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-[var(--border)] bg-[var(--surface)] px-4 py-3 sm:px-6"><Link href="/app/dashboard" aria-label="Auralis Finance" className="md:hidden"><Logo className="h-6" /></Link><div className="hidden h-9 w-full max-w-md items-center gap-2 rounded-[10px] border border-[var(--border)] bg-[var(--surface-muted)] px-3 text-sm text-[var(--text-secondary)] lg:flex"><Search size={14}/><span className="flex-1">Search any asset, protocol, or page…</span><kbd className="rounded border border-[var(--border)] bg-[var(--surface)] px-1.5 py-0.5 text-[11px]">⌘K</kbd></div><span className="text-sm text-[var(--text-secondary)] lg:ml-auto">Mantle Mainnet · chainId 5000</span><ThemeToggle /><WalletButton /></header><RouteTransition><div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">{children}</div></RouteTransition><footer className="mt-4 border-t border-[var(--border)] px-4 py-6 sm:px-6"><ContractsFooterStrip /></footer></main>
  </div>;
}
