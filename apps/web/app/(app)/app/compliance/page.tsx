...
import { Button, Card, CardContent, CardHeader, CardTitle, ConfidenceMeter, EligibilityChip, EmptyState, ErrorState, ProofCard, Skeleton, StateWrapper, StatusPill } from "@auralis/ui";
import { addresses, complianceAttestorAbi } from "../../../../lib/contracts";
import { explorerTxUrl } from "../../../../lib/deployments";
import { idHash, verdictCode } from "../../../../lib/proof";
...
export default function CompliancePage() {
  ...
  const [attestations, setAttestations] = useState<{ assetClass: string; verdict: Verdict; validUntil: string; proof: string; txHash: string }[]>([]);
  ...
  async function runScan(stale = false) {
    ...
    setSelected(null);
    try {
      const res = await fetch("/api/compliance/scan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ wallet: activeWallet, jurisdiction }) });
  ...
  function exportReport() {
    ...
    a.href = url;
    a.download = `auralis-compliance-${activeWallet.slice(0, 8)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }

  async function mintAttestation() {
    const usdy = report?.results.find((r) => r.assetClass === "US_TREASURY_RWA") ?? selected;
    if (!usdy || !report || !address) return;
    const txHash = await writeContractAsync({ address: addresses.complianceAttestor, abi: complianceAttestorAbi, functionName: "mintAttestation", args: [address, idHash(usdy.assetClass), verdictCode(usdy.verdict), report.reportHash as `0x${string}`, idHash(jurisdiction), "ipfs://pending/usdy-compliance", 60n * 60n * 24n * 30n] });
    await fetch("/api/compliance/attestation", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ wallet: address, assetClass: usdy.assetClass, verdict: usdy.verdict, checkHash: report.reportHash, txHash, validUntil: usdy.validUntil }) });
    setAttestations((rows) => [{ assetClass: usdy.assetClass, verdict: usdy.verdict, validUntil: usdy.validUntil ?? new Date(Date.now() + 30 * 86400_000).toISOString(), proof: report.reportHash, txHash }, ...rows]);
    setMintOpen(false);
    setTab("Attestations");
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    {tab === "Wallet Scan" && <StateWrapper status={status === "idle" ? "populated" : status === "loading" ? "loading" : status === "error" ? "error" : status === "stale" ? "stale" : "populated"}><section className="grid gap-4 lg:grid-cols-[1fr_.9fr]">
      <Card><CardHeader><CardTitle>Run / Re-scan wallet</CardTitle></CardHeader><CardContent className="space-y-4"><label className="grid gap-1 text-sm">Wallet<input value={activeWallet} onChange={(e)=>setWallet(e.target.value)} placeholder="Connect wallet" className="h-10 rounded-[10px] border border-[var(--border)] bg-[var(--surface)] px-3 font-mono focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--teal)]" /></label><label className="grid gap-1 text-sm">Declared jurisdiction<input value={jurisdiction} onChange={(e)=>setJurisdiction(e.target.value.toUpperCase())} className="h-10 rounded-[10px] border border-[var(--border)] bg-[var(--surface)] px-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--teal)]" maxLength={8} /></label><div className="flex gap-2"><Button onClick={()=>runScan(false)} disabled={status==="loading"}>{status==="loading"?"Scanning…":"Run scan"}</Button><Button variant="secondary" onClick={()=>runScan(true)}>Mark stale re-scan</Button><Button variant="ghost" onClick={()=>{setReport(null);setStatus("empty");}}>Empty state</Button></div></CardContent></Card>
    ...
    </section></StateWrapper>}

    {tab === "Eligibility Matrix" && <StateWrapper status={report ? "populated" : "empty"}><section className="grid gap-4 lg:grid-cols-[1fr_360px]"><Card><CardHeader><CardTitle>RWAs × {jurisdiction || "Jurisdiction"}</CardTitle></CardHeader><CardContent>{!report ? <EmptyState title="Run a scan to populate the matrix" action={<Button onClick={()=>setTab("Wallet Scan")}>Go to scan</Button>} /> : <div className="overflow-hidden rounded-xl border border-[var(--border)]"><table className="w-full text-sm"><thead className="bg-[var(--surface-muted)] text-left"><tr><th className="p-3">Asset class</th><th className="p-3">Example</th><th className="p-3">Verdict</th><th className="p-3">Confidence</th></tr></thead><tbody>{assetRows.map((row)=>{const result=resultsByClass.get(row.classId as EligibilityResult["assetClass"]) ?? { wallet: activeWallet, assetClass: row.classId as EligibilityResult["assetClass"], verdict: "NOT_CHECKED" as Verdict, reasons: ["No scan result for this class yet."], confidence: 0}; return <tr key={row.classId} onClick={()=>setSelected(result)} className="cursor-pointer border-t border-[var(--border)] hover:bg-[var(--teal-wash)]"><td className="p-3 font-medium">{row.label}</td><td className="p-3 text-[var(--text-secondary)]">{row.example}</td><td className="p-3"><EligibilityChip verdict={result.verdict}/></td><td className="p-3">{result.confidence}%</td></tr>})}</tbody></table></div>}</CardContent></Card><SidePanel result={selected} /></section></StateWrapper>}

    {tab === "Compliance Report" && <StateWrapper status={report ? "populated" : "empty"}><section className="grid gap-4 lg:grid-cols-[1fr_340px]"><Card><CardHeader><CardTitle>Exportable report</CardTitle></CardHeader><CardContent>{!report ? <EmptyState title="No report to export" action={<Button onClick={()=>setTab("Wallet Scan")}>Run scan</Button>} /> : <div className="space-y-4"><div className="grid gap-3 md:grid-cols-4"><Metric label="Report" value={report.reportId.slice(0,18)+"…"}/><Metric label="Methodology" value={`v${report.methodologyVersion}`}/><Metric label="Generated" value={new Date(report.generatedAt).toLocaleDateString()}/><Metric label="Jurisdiction" value={report.walletScreen.jurisdiction}/></div><div className="grid gap-2">{report.results.map((r)=><div key={r.assetClass} className={`rounded-xl border p-3 ${verdictTone(r.verdict)}`}><div className="flex items-center justify-between"><span className="font-medium">{r.assetClass}</span><EligibilityChip verdict={r.verdict}/></div><p className="mt-2 text-sm text-[var(--text-secondary)]">{r.reasons.join(" ")}</p></div>)}</div></div>}</CardContent></Card><Card><CardHeader><CardTitle>Actions</CardTitle></CardHeader><CardContent className="space-y-3"><Button className="w-full" disabled={!report} onClick={exportReport}>Export PDF</Button><Button className="w-full" disabled={!report} onClick={()=>setMintOpen(true)}>Mint attestation</Button>{exported && <p className="text-xs text-[var(--emerald)]">Export generated as JSON report artifact for this build step.</p>}{report && <ProofCard label="Report hash" hash={report.reportHash}/>}</CardContent></Card></section></StateWrapper>}

    {tab === "Attestations" && <StateWrapper status={attestations.length ? "populated" : "empty"}><section className="space-y-4"><div className="flex justify-between"><h2 className="font-display text-2xl">Attestation history</h2><Button onClick={()=>setMintOpen(true)} disabled={!report}>Mint new attestation</Button></div><Card><CardContent className="pt-5"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-[var(--text-secondary)]"><th className="p-3">Asset class</th><th className="p-3">Verdict</th><th className="p-3">Valid until</th><th className="p-3">Proof</th><th className="p-3">Explorer</th><th className="p-3">Action</th></tr></thead><tbody>{attestations.map((a,i)=><tr key={`${a.assetClass}-${i}`} className="border-t border-[var(--border)]"><td className="p-3 font-medium">{a.assetClass}</td><td className="p-3"><EligibilityChip verdict={a.verdict}/></td><td className="p-3">{new Date(a.validUntil).toLocaleDateString()}</td><td className="p-3 font-mono">{a.proof.slice(0,10)}…</td><td className="p-3">{a.txHash && a.txHash.startsWith("0x") ? <a className="text-[var(--teal)]" href={explorerTxUrl(a.txHash)} target="_blank" rel="noreferrer">Open</a> : <span className="text-[var(--text-secondary)]">Pending</span>}</td><td className="p-3"><Button size="sm" variant="secondary">Revoke</Button></td></tr>)}</tbody></table></div></CardContent></Card></section></StateWrapper>}

    {mintOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4"><Card className="w-full max-w-lg"><CardHeader><CardTitle>Mint compliance attestation</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-[var(--text-secondary)]">This opens the user-signed attestation flow. Your wallet signs <code className="font-mono">mintAttestation</code> before anything is submitted to Mantle.</p><div className="rounded-xl bg-[var(--surface-muted)] p-3 text-sm">Contract: AuralisComplianceAttestor.mintAttestation<br/>Network: Mantle Mainnet · chainId 5000.</div><div className="flex justify-end gap-2"><Button variant="secondary" onClick={()=>setMintOpen(false)}>Cancel</Button><Button disabled={!address || txPending} onClick={mintAttestation}>{txPending ? "Open wallet…" : "Mint attestation"}</Button></div></CardContent></Card></div>}
  </div>;
}
